...
function renderInHand(){
    if(myGameArea.keys[32]){
        ...
        if(currentDirection == 1){
            ...
            myGameArea.context.translate(700 + 350 + 20 * Math.cos(ng),350 + 20 * Math.sin(ng));
            ...
            myGameArea.context.save();
            myGameArea.context.translate(700 + 350 + 20 * Math.cos(ng),370 + 20 * Math.sin(ng));
            myGameArea.context.rotate(ng);
            myGameArea.context.drawImage(imt,32 * chooseBlock(inventory[selectIndex][0])[0],32 * chooseBlock(inventory[selectIndex][0])[1],32,32,0, -64, 64, 64);
            myGameArea.context.restore();
        }
...
function renderData(){
    ...
    if(datashow){
        ...
        myGameArea.context.fillRect(0,0,2100,200);
        myGameArea.context.globalAlpha = 1;
        myGameArea.context.fillStyle = 'white';
        myGameArea.context.font = '20px Arial';
        myGameArea.context.fillText('X: ' + xpos,700 + 10,30);
        myGameArea.context.fillText('Y: ' + ypos,700 + 10,60);
        myGameArea.context.fillText('Current biome: ' + (biomes[Math.round(ypos) * width + Math.round(xpos)][0].toUpperCase() + biomes[Math.round(ypos) * width + Math.round(xpos)].slice(1)),700 + 10,90);
        myGameArea.context.fillText('Standing on: ' + data.viewdata[blocks[Math.round(ypos)][Math.round(xpos)]].split('*')[0],700 + 10,150);
        myGameArea.context.fillText('Looking at: ' + (data.viewdata[blocks[Math.round(ypos)][((currentDirection == 1) ? (Math.ceil(xpos) + 1) : (Math.floor(xpos) - 1))]] ?? 'Void').split('*')[0],700 + 10,120);
    }
}
function renderPlayerArmor(moved) {
    ...
    if (data.armordraw[equipped.h[0]]) {
        myGameArea.context.drawImage(ima, CYCLE_LOOP[currentLoopIndex] * 32, data.armordraw[equipped.h[0]][charRow] * 32, 32, 32, 700 + 302, 302, 96, 96);
    }
    if (data.armordraw[equipped.b[0]]) {
        myGameArea.context.drawImage(ima, CYCLE_LOOP[currentLoopIndex] * 32, data.armordraw[equipped.b[0]][charRow] * 32, 32, 32, 700 + 302, 302, 96, 96);
    }
    if (data.armordraw[equipped.l[0]]) {
        myGameArea.context.drawImage(ima, CYCLE_LOOP[currentLoopIndex] * 32, data.armordraw[equipped.l[0]][charRow] * 32, 32, 32, 700 + 302, 302, 96, 96);
    }
    if (data.armordraw[equipped.c[0]]) {
        myGameArea.context.drawImage(ima, CYCLE_LOOP[currentLoopIndex] * 32, data.armordraw[equipped.c[0]][charRow] * 32, 32, 32, 700 + 302, 302, 96, 96);
    }
...
function renderHealth() {
    for (var x = 0; x < Math.ceil(playerHealth); x++) {
        myGameArea.context.drawImage(imh, (Math.ceil(playerHealth) % 2 == 1 && x == Math.ceil(playerHealth) - 1 ? 96 :(x % 2 >= 1 ? 64 : 32)), 0, 32, 32,700 + 6 + 16 * (x % 20), 568 - 32 * Math.floor(x / 20), 32, 32);
    }
    for (var x = Math.ceil(playerHealth); x < maxHealth; x++) {
        myGameArea.context.drawImage(imh, (maxHealth % 2 == 1 && x == maxHealth - 1 ? 192 : (x % 2 >= 1 ? 160 : 128)), 0, 32, 32,700 + 6 + 16 * (x % 20), 568 - 32 * Math.floor(x / 20), 32, 32);
    }
...
function renderDrops() {
    for (var b = 0; b < dropblocs.length; b++) {
        ...
        if (dropblocs[b].num > 0) {
            myGameArea.context.drawImage(imt, dq[0] * 32, dq[1] * 32, 32, 32,700 + (dropblocs[b].xp - xpos + (3.34 + 14/96)) * 96, (dropblocs[b].yp - ypos + (3.34 + 14/96)) * 96, 32, 32);
            var k = 0;
...
function renderBlocks() {
    ...
    for (var r = Math.floor(ypos) - 6; r < Math.floor(ypos) + 6; r++) {
        for (var c = Math.floor(xpos) - 12; c < Math.floor(xpos) + 14; c++) {
            if (r >= 0 && c >= 0 && r < 1000 && c < 1000) {
                if(data.partial.includes(blocks[r][c])){
                    ctx.drawImage(imt,32 * chooseBlock(data.defaultTile[biomes[r * width + c]])[0], 32 * chooseBlock(data.defaultTile[biomes[r * width + c]])[0], 32, 32,700 +  (c - xpos + (3 + 14/96)) * 96, (r - ypos + (3 + 14/96)) * 96, 96, 96);
                }
                var d = chooseBlock(blocks[r][c]);
                ctx.drawImage(imt, d[0] * 32, d[1] * 32, 32, 32,700 +  (c - xpos + (3 + 14/96)) * 96, (r - ypos + (3 + 14/96)) * 96, 96, 96);
            }
        }
    }
}
...
function renderHotbar() {
    for (var w = 0; w < 7; w++) {
        myGameArea.context.drawImage(ime, w == selectIndex ? 140 : 0, 0, 70, 70,700 +  105 + (70 * w), 600, 70, 70);
        if (inventory[w]) {
            var gx = chooseBlock(inventory[w][0])[0] * 32, gy = chooseBlock(inventory[w][0])[1] * 32;
            myGameArea.context.drawImage(imt, gx, gy, 32, 32,700 +  108 + (70 * w), 603, 64, 64);
            if (inventory[w][1] > 1) {
                myGameArea.context.font = '20px Arial';
                myGameArea.context.fillStyle = 'white';
                myGameArea.context.fillText(inventory[w][1],700 +  145 + w * 70, 665);
            }
        }
    }
}
...
function renderDeath() {
    ...
    myGameArea.context.fillRect(700 + 0, 0, 700, 700);
    myGameArea.context.font = '50px Arial';
    myGameArea.context.fillStyle = 'white';
    myGameArea.context.fillText('You died!',700 +  250, 200);
    myGameArea.context.fillStyle = '#555';
    myGameArea.context.fillRect(700 + 300, 500, 100, 50);
    myGameArea.context.font = '22px Arial';
    myGameArea.context.fillStyle = 'white';
    myGameArea.context.fillText('Respawn',700 +  305, 530);
...
function renderTitleScreen(){
    myGameArea.context.drawImage(bImage,700 +  0,0, 700, 700);
    myGameArea.context.fillStyle = 'lightgrey';
    myGameArea.context.fillRect(700 + 250,250,200,75);
    myGameArea.context.fillRect(700 + 250,350,200,75);
    myGameArea.context.fillStyle = 'black';
    myGameArea.context.fillRect(700 + 253,253,194,69);
    myGameArea.context.fillRect(700 + 253,353,194,69);
    myGameArea.context.fillStyle = 'lightgrey';
    myGameArea.context.fillRect(700 + 255,255,190,65);
    myGameArea.context.fillRect(700 + 255,355,190,65);
    myGameArea.context.fillStyle = 'black';
    myGameArea.context.font= '35px Arial';
    myGameArea.context.fillText('New game',700 + 260,300);
    myGameArea.context.fillText('Load game',700 + 260,400);
...
function renderHealTimeout(){
    if(healTimeout > 0){
        healTimeout -= 0.02;
        myGameArea.context.drawImage(imh,128,32,32,32,700 + 668,32,32,32);
        myGameArea.context.globalAlpha = 0.6;
        myGameArea.context.fillStyle = 'white';
        myGameArea.context.beginPath();
        myGameArea.context.moveTo(700 + 684,48);
        myGameArea.context.lineTo(700 + 684,32);
        for(var x = 0; x < Math.ceil(healTimeout); x++){
            myGameArea.context.lineTo(700 + 684 + 16 * Math.cos((x * 6 * (Math.PI/180)) - Math.PI / 2),48 + 16 * Math.sin((x * 6 * (Math.PI/180)) - Math.PI / 2));
        }
